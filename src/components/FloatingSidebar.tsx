            ...
            {items.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="text-xs font-medium whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity duration-200 pr-4">
                    {item.label}
                  </span>
                </a>
              );
      ...
      <AnimatePresence>
        {visible && (
          <motion.div
            ...
            exit={{ opacity: 0, y: 20 }}
            transition={{ duration: 0.3 }}
            className="lg:hidden fixed bottom-6 left-1/2 -translate-x-1/2 z-40 flex items-center gap-2 px-4 py-2.5 rounded-full bg-[#1a1a1a]/95 border border-white/10 backdrop-blur-xl shadow-2xl shadow-black/60"
          >
            {items.map((item) => {
              const Icon = item.icon;
              return (
